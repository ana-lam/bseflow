...
def plotSankey(df, title="", CEE=False, formation_channel=False, save_path=None, custom_trash=None):
    ...
    fig.update_layout(title_text=f'{title} Model', font=dict(size=20))
    if save_path:
        fig.write_html(f'{save_path}')
    else:
        fig.write_html(f'/Users/ana/Documents/BNS_formation/sankey_htmls/{df.columns[0].split("_")[0]}.html')